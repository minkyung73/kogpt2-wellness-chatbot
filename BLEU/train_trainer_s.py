...
class KoGPT2Chat(LightningModule):
    ...
    def train_dataloader(self):
        data = pd.read_csv('chatbot_dataset_train.csv')
            ...
            while 1:
              ...
              while 1:
                  ...
                  if gen == EOS:
                    break
                  a += gen.replace('▁', ' ')
              candidate = mecab.morphs(a)
              # print('candidate: ', candidate)
              print("Chatbot(BLEU: {}) > {}".format(sentence_bleu(reference, candidate), a.strip()))
              # print(f"Chatbot(BLEU: {sentence_bleu(reference, a.strip()):.2f}) >", a.strip())
...
if __name__ == "__main__":
    if args.train:
        checkpoint_callback = ModelCheckpoint(
            dirpath='model_chp',
            filename='train_{epoch:02d}-{train_loss:.2f}',
            verbose=True,
            save_last=True,
            monitor='train_loss',
            mode='min',
